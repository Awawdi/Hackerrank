...
# Define regex patterns for extracting identifier, question text, and relative time
identifier_pattern = r'<h3><a href="/questions/(\d*)'
question_text_pattern = r'class="question-hyperlink">(.*)</a>'
relative_time_pattern = r'<span title=".*" class="relativetime">(.*)<'

# Find all matches for each pattern in input 
identifiers = re.findall(identifier_pattern, html_content)
questions = re.findall(question_text_pattern, html_content)
times = re.findall(relative_time_pattern, html_content)

# Print the extracted information in the format
for iden, text, time in zip(identifiers, questions, times):
    print(f"{iden};{text};{time}")
